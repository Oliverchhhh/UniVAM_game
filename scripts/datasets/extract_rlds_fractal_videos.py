"""
Extract per-episode MP4 videos from the Fractal20220817 RLDS dataset.

Fractal20220817 (ucasmichael/fractal20220817_data) is an RLDS-format dataset with:
  - Each tfrecord shard contains multiple full episodes
  - Camera keys in steps/observation: image, image_depth
  - Each episode has attributes (task_family, env_name, etc.) and aspects (success, feasible, etc.)
  - Split: train only

Example:
    python scripts/datasets/extract_rlds_fractal_videos.py
    python scripts/datasets/extract_rlds_fractal_videos.py -c image_depth -s 0 10
"""
...
def extract_dataset_videos(
    ...
    for shard_idx in range(shard_start, shard_end):
        ...
        for ep_i, record in enumerate(raw_ds):
            ...
            out_path = episode_output_dir / f"shard{shard_idx:04d}_ep{ep_i:06d}.mp4"
...
def main():
    parser = argparse.ArgumentParser(
        description="Extract per-episode MP4 videos from Fractal20220817 RLDS dataset",
    )
    default_data_root = Path(
        os.environ.get("DATASETS_PATH", ".")
    ) / "ucasmichael" / "fractal20220817_data" / "fractal20220817_data" / "0.1.0"
    parser.add_argument(
    ...
    default_output_root = Path(
        os.environ.get("DATASETS_PATH", ".")
    ) / "ucasmichael" / "fractal_videos"
    parser.add_argument(
        "-o", "--output-root", type=Path, default=default_output_root,
        help=f"Directory to save output videos (default: {default_output_root})",
    )
    parser.add_argument(
        "-c", "--camera-key", default="image",
        help="Camera key in steps/observation (default: image; also available: image_depth)",
    )
    parser.add_argument(
